...
export const RegisterOtp = () => {
  ...
  // Decode email from the URL
  let email;
  try {
    const encodedEmail = location.pathname.split('/')[2]; // Extract encoded email from the URL
    email = atob(decodeURIComponent(encodedEmail)); // Decode Base64 and URI encoding
  } catch (error) {
    console.error('Failed to decode email from URL:', error);
    toast.error('Invalid email in the URL.');
    navigate('/'); // Redirect to home if email is invalid
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`/api/auth/signupvalidate-otp/${email}`, { otp: finalOtp });
  ...
  const handleResendOTP = async () => {
    ...
    try {
      await axios.post('/api/auth/signupresend-otp', { email }); // Send email with resend OTP request
      ...
      <div className="flex p-9 max-w-md mx-auto flex-col md:flex-row md:items-center gap-5">
        <div className="flex-1">
          <form
            ...
            className="flex flex-col gap-4 p-6 rounded shadow-2xl drop-shadow-lg filter shadow-gray-800 dark:shadow-purple-500 w-80"
          >
            ...
            >
              {loading ? (
                <>
                  <Spinner size="sm" />
                  <span className="pl-3">Validating...</span>
                </>
              ) : (
                'Validate OTP'
              )}
